...
在 `dsh-access` 内提供统一的远程访问能力：Access management 设置卡片保留公共状态与网关端口配置，并拆成“账号与权限”和“远程访问”两个 Tab；所有局域网二维码和 Cloudflare 临时隧道都指向 Access management 登录网关，访问前必须登录，不再依赖或暴露独立的 Pocket 3081 入口。

# Scope

- 在现有 Access management 设置卡片中加入两个 Tab：“账号与权限”和“远程访问”。
...
- 不提供绕过 Access management 登录的匿名入口。
...
- A1：Access management 设置卡片显示“账号与权限”和“远程访问”两个 Tab，默认打开“账号与权限”。
...
- A7：手机或另一台电脑打开 LAN URL 时，未登录请求进入 访问管理登录页；登录后的 HTTP、SSE 和 WebSocket 继续经过现有账号与权限检查。
...
- A11：Cloudflare 隧道始终指向当前 访问管理网关端口，不能指向 3080 或 3081；关闭后 cloudflared 子进程退出且 URL 清空。
...
- A18：`npm test` 与 `npm run build` 通过；本机 3080 直连保持可用，局域网电脑可通过 访问管理网关完成登录，公网隧道可完成登录和 WebSocket 流式通信。

# Constraints and invariants

- Node.js 版本要求保持 `>=22.5`。
- 访问管理网关是唯一远程入口；3080 保持 loopback，3081 不由本插件监听。
...
- 二维码仅编码 URL，不承担认证功能。
- cloudflared 文件和运行状态只保存在 Access management 数据目录内。
...
- Cloudflare 临时隧道只连接 访问管理网关，且控制权仅属于 Admin。
