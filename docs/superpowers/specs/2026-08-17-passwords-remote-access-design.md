# DSH Access management 远程访问整合设计
...
- 访问管理网关：默认端口 3088，访问前需要登录。
- Pocket 代理：默认端口 3081，负责 Host/Origin 改写、二维码、隧道和移动端适配。

本变更把 Pocket 的远程访问能力整合进 Access management，以 3088 访问管理网关作为唯一远程安全入口，不再需要独立的 3081 代理。

## 2. 目标

1. 在现有 Access management 设置卡片中提供两个 Tab：
   - `账号与权限`
   - `远程访问`
...
5. 在 Access management 客户端独立实现移动端布局适配，并保留现有非安全上下文兼容补丁。
6. 局域网和公网访问都必须经过 Access management 登录和权限检查。

## 3. 非目标

- 不保留或兼容独立的 Pocket 3081 代理进程。
- 不新增绕过 Access management 登录的匿名访问模式。
...
2. 局域网访问卡片
   ...
   - URL 二维码
   - `复制地址` 按钮
   - 提示：扫码后先进入 访问管理登录页，认证成功后再打开 DSH
3. 公网临时访问卡片
   - Cloudflare 临时隧道开关
   - 下载、启动、运行、停止和错误状态
...
1. 校验端口。
2. 调用现有网关端口保存 API。
3. 重启 Access management 登录网关，保持 3080 本机 DSH Web 不变。
...
Cloudflare 临时隧道只指向 访问管理网关：

```text
公网浏览器 -> trycloudflare HTTPS URL -> 访问管理网关 -> DSH Web
```

### 5.2 服务端模块

按确认行为在 Access management 中独立实现的能力（不复制 Pocket GPL-2.0 服务端代码）：
...
1. LAN URL 和公网隧道都必须落到 访问管理网关，不能直接转发到 3080。
...
7. cloudflared 下载文件必须限定到 Access management 数据目录，并验证平台、文件类型和可执行权限。
...
- Access management 安装后不再要求安装 `dsh-pocket`。
...
- `http://127.0.0.1:<port>` 和 `http://<LAN_IP>:<port>` 都进入 访问管理登录页。
...
本变更完成后，Password 插件具备 Pocket 的核心远程访问体验：二维码、临时公网隧道和移动端适配，同时所有远程访问都经过 Access management 的认证与权限边界。独立 Pocket 插件不再是运行依赖。
