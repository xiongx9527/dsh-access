...
test('account trigger uses the same wide-row and circular rail class pattern as Settings', () => {
  ...
  assert.match(wide, /class="dshpw-account-trigger"/);
  assert.match(rail, /class="dshpw-account-trigger rail"/);
});
...
test('account popover escapes sidebar clipping through a fixed body portal', () => {
  ...
  assert.match(clientSource, /\.dshpw-account-popover\{position:fixed;/);
});


test('logout button keeps a visible red danger style even when the host error token is absent', () => {
  const clientSource = readFileSync(new URL('../src/client/index.tsx', import.meta.url), 'utf8');
  assert.match(clientSource, /\.dshpw-account-logout\{[^}]*var\(--dsw-alias-label-error,#ef4444\)/);
  assert.match(clientSource, /\.dshpw-account-logout:hover[^\{]*\{[^}]*rgba\(239,68,68,/);
});
...
test('workspace summary wraps the complete assigned path instead of truncating it', () => {
  const accountSource = readFileSync(new URL('../src/client/account.tsx', import.meta.url), 'utf8');
  const clientSource = readFileSync(new URL('../src/client/index.tsx', import.meta.url), 'utf8');
  assert.match(accountSource, /className: 'dshpw-account-workspace'/);
  assert.match(clientSource, /\.dshpw-account-summary dd\.dshpw-account-workspace\{[^}]*overflow-wrap:anywhere/);
});
...
test('account center shows the compact subuser list immediately without a count-only gate', () => {
  ...
  assert.match(accountSource, /className: 'dshpw-admin-user-list'/);
  assert.match(clientSource, /\.dshpw-admin-user-list\{[^}]*max-height:[^;}]+;[^}]*overflow:auto/);
});
...
test('legacy settings user rows expand one inline permission editor at a time', () => {
  ...
  assert.match(cardSource, /className: 'dshpw-perm dshpw-user-permission-editor'/);
...
test('modern account center uses the same one-user-at-a-time permission accordion', () => {
  ...
  assert.match(accountSource, /className: 'dshpw-admin-user-details'/);
  assert.match(accountSource, /className: 'dshpw-admin-user-delete'/);
});


test('modern account user rows show the most recent login time', () => {
  const accountSource = readFileSync(new URL('../src/client/account.tsx', import.meta.url), 'utf8');
  assert.match(accountSource, /className: 'dshpw-admin-user-last-login'/);
...
test('quota inputs have visible labels in both settings and account management', () => {
  ...
  for (const source of [accountSource, cardSource]) {
    assert.match(source, /className: 'dshpw-limit-field'/);
    assert.match(source, /className: 'dshpw-limit-label'/);
