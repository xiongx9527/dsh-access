...
#
# 用法（二选一）:
#   1) curl 直接装:  curl -fsSL https://raw.githubusercontent.com/xiongx9527/dsh-access/main/install.sh | bash
#   2) 先 clone 再装: git clone https://github.com/xiongx9527/dsh-access && cd dsh-access && bash install.sh
...
if [ "$(id -u)" = "0" ]; then
  DEST="${DSH_ACCESS_DIR:-/opt/dsh-access}"
else
  DEST="${DSH_ACCESS_DIR:-$HOME/dsh-access}"
fi
if [ -d "$DEST" ]; then
